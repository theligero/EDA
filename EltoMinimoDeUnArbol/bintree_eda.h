...
public:
   ...
   T menorElto(bintree<T> const& node) const {
       if (!node.left().empty() && !node.right().empty()) {
           T aux = node.root();
           T hijoIzq = menorElto(node.left());
           T hijoDer = menorElto(node.right());
           if (aux < hijoIzq && aux < hijoDer) return aux;
           else if (hijoIzq < aux && hijoIzq < hijoDer) return hijoIzq;
           else return hijoDer;
       }
       else if (!node.left().empty() && node.right().empty()) {
           T aux = node.root();
           T hijoDer = menorElto(node.left());
           if (aux > hijoDer) return hijoDer;
           else return aux;
       }
       else if (node.left().empty() && !node.right().empty()) {
           T aux = node.root();
           T hijoIzq = menorElto(node.right());
           if (aux > hijoIzq) return hijoIzq;
           else return aux;
       }
