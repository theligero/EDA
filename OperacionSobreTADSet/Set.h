...
public:
    ...
    // función de tipo lineal (en el peor de los casos el
    // operator<= recorre todo el vector y devuelve verdadero
    // en caso de que sean iguales, falso en caso contrario
    bool operator<=(const Set<T>& set) const {
        int aux1 = 0, aux2 = 0;

        while (aux1 < this->nelems && aux2 < set.nelems) {
            if (this->array[aux1] == set.array[aux2]) { ++aux1; ++aux2; }
            else if (this->array[aux1] < set.array[aux2]) return false;
            else if (this->array[aux1] > set.array[aux2]) { ++aux2; }
        }

        return aux1 == this->nelems;
    }
