...
public:
   ...
   // O(n), siendo n el número de eltos del árbol
   void laFrontera(bintree<T> const& node, std::vector<T>& aux) {
       if (node.empty()) return;
       if (!node.left().empty()) laFrontera(node.left(), aux);
       if (!node.right().empty()) laFrontera(node.right(), aux);
       if (node.left().empty() && node.right().empty())
           aux.push_back(node.root());
   }
};
