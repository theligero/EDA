...
public:
    ...
    // Operación de unión de conjuntos de complejidad
    // O(N), siendo N el número de eltos del set de mayor
    // tamaño
    Set<T> operator||(const Set<T>& other) const {
        ...
        while (i < this->nelems || j < other.nelems) {
            if (i < this->nelems && j < other.nelems) {
                if (this->array[i] < other.array[j]) {
                    aux.add(this->array[i]); ++i;
                }
                else if (this->array[i] == other.array[j]) {
                    aux.add(this->array[i]); ++i; ++j;
                }
                else {
                    aux.add(other.array[j]); ++j;
                }
            }
            else {
                if (i == this->nelems) {
                    while (j < other.nelems) {
                        aux.add(other.array[j]);
                        ++j;
                    }
                }
                else {
                    while (i < this->nelems) {
                        aux.add(this->array[i]);
                        ++i;
                    }
                }
            }
    ...
    // Operación de intersección de conjuntos de complejidad
    // O(N), siendo N el número de eltos del set de menor
    // tamaño
    Set<T> operator&&(const Set<T>& other) const {
        Set<T> aux;
        int i = 0, j = 0;
